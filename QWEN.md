...
**Event publishing** — `events/` package provides `EventPublisher` implementations (`DefaultHouseholdEventPublisher`, `StatsEventPublisher`) composed via `CompositeTransactionEventPublisher`. `ItemEvent<T>` is the generic event record.
