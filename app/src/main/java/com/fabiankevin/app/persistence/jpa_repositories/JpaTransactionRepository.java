...
public interface JpaTransactionRepository extends JpaRepository<TransactionEntity, UUID> {
    @Query("""
                ...
                WHERE YEAR(t.transactionDate) = :year and t.account.userId = :userId
                GROUP BY t.category.name
            """)
    Streamable<SummaryPointProjection> getSummaryByYearAndUserIdGroupedByCategory(@Param("year") int year, @Param("userId") UUID userId);
}
